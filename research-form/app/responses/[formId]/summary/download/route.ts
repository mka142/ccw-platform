...
function parseItemObjectsToStrings(item: any): any {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const newItem: any = {};
...
export async function GET(
  ...
  try {
    const indexesData = await getResponseIdIndexMap(formId);
    if (!indexesData) {
    ...
    const cacheData = unstable_cache(
      async (formId) => await downloadAllResponses(formId),
    ...
    if (type === "json") {
      ...
      const musicSliderFields: string[] = formSchema.formPages.flatMap((page) =>
        page.fields
          .filter((field) => field.type === "musicSlider")
          .map((field) => field.id)
      ) as string[];
      ...
      const orderedData = data.map((i) => {
        const item = parseItemObjectsToStrings(i.formData);
        const orderedItem: Record<string, string> = {};
        // Add index field
        orderedItem["index"] = `Odpowiedź ${indexesData[i._id as string] + 1}`;
        ...
        Object.keys(formSchemaIdTitleMap).forEach((key) => {
          if (musicSliderFields.includes(key)) {
            ...
            orderedItem[key] =
              item[key] !== undefined ? item[key].toString() : "";
          }
        });
      ...
      XLSX.utils.sheet_add_aoa(
        worksheet,
        [["index", ...Object.values(formSchemaIdTitleMap)]],
        { origin: "A1" }
      );
      ...
      musicSliderFields.forEach((fieldId) => {
        const musicSliderData = formData
          .map((item, index) => {
